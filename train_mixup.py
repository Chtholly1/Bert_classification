...
from conf.config import Config, args, MODEl_NAME, categories
...
def main(args):
    ...
    elif MODEl_NAME == './resource/base_models/base_albert':
        #model = AlBertModel(config).to(device)
        model = AlBertModelMixup(config).to(device)
    elif MODEl_NAME == './resource/base_models/base_roberta':
        model = BertModel(config).to(device)
    #model = nn.DataParallel(model, device_ids=[0, 1]) #多显卡并行计算
