...
class MyDataSet(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        tokens = self.tokenizer(text, max_length=MAX_LEN, padding='max_length',
                                   truncation=True)
        input_ids = tokens['input_ids']
        attention_mask = tokens['attention_mask']

        return t.tensor(input_ids, dtype=t.long), t.tensor(attention_mask, dtype=t.long), t.tensor(label, dtype=t.long)
...
def load_data(file_path):
    ...
    with open(file_path) as f:
        for line in f:
            ...
            if label not in label_2_ids:
                continue
            data_list.append([label_2_ids[label], text])
    return data_list
